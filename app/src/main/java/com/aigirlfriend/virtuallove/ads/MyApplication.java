...
public class MyApplication extends Application implements Application.ActivityLifecycleCallbacks, LifecycleObserver {
    ...
    private static final String PREF_SPLASH_TAP_COUNT = "splash_tap_count";
    ...
    public boolean incrementSplashTapCount() {
        int currentTaps = sharedPreferencesInApp.getInt(PREF_SPLASH_TAP_COUNT, 0);
        int newCount = currentTaps + 1;
        editorInApp.putInt(PREF_SPLASH_TAP_COUNT, newCount).apply();
        return newCount >= REQUIRED_TAPS;
    }
    ...
    private void initAds() {
        ...
        AperoAdConfig config = new AperoAdConfig(this, BuildConfig.API_KEY_ADS,
                AperoAdConfig.PROVIDER_ADMOB, environment);

        config.setListDeviceTest(Arrays.asList(
                BuildConfig.DEVICE_ID_TEST,
                BuildConfig.EMULATOR_ID_TEST
        ));
        ...
        com.ads.control.config.AdjustConfig adjustConfig = new com.ads.control.config.AdjustConfig(ADJUST_TOKEN);
        ...
        config.setIdAdResume(BuildConfig.ads_inter_splash_open);
        config.setIdAdResumeMedium(BuildConfig.ads_inter_splash_open_high);
        config.setIdAdResumeHigh(BuildConfig.ads_inter_splash_open_high);
